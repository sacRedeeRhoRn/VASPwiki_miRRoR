<!-- Source: https://vasp.at/wiki/index.php/Calculate_U_for_LSDA%2BU | revid: 36491 | retrieved: 2026-06-24 -->
...
This trick breaks the symmetry of the Ni sub-lattice and allows us to
treat atom 1 differently from atom 2-15. Our
[POTCAR](../input-files/POTCAR.md) file has to reflect the fact that we now
formally have 3 "species" (2 ×Ni + 1×O), *i.e.*, we concatenate two Ni
[POTCAR](../input-files/POTCAR.md) files and one O
[POTCAR](../input-files/POTCAR.md) file:

    cat Ni/POTCAR Ni/POTCAR O/POTCAR > POTCAR

To check whether you have a suitable [POTCAR](../input-files/POTCAR.md)
type:
...
Instrumental here is that we correctly specify the initial magnetic
moments (by means of [MAGMOM](../incar-tags/MAGMOM.md)-tag). The setting
above is consistent with the AFM-II magnetic structure: alternating
ferromagnetic Ni (111)-layers.

Secondly, we set [LORBIT](../incar-tags/LORBIT.md)`=11`: at the end of the
[OUTCAR](../output-files/OUTCAR.md) file, VASP will write the number of
(*d*-) electrons per site. This information we will need to compute the
*U*-parameter.

Last but not least, we set [LMAXMIX](../incar-tags/LMAXMIX.md)`=4`: this
is needed to be able to perform non-selfconsistent
([ICHARG](../incar-tags/ICHARG.md)`=11`) DFT+U calculations
([LDAUTYPE](../incar-tags/LDAUTYPE.md)`=3`) in the following. For this
reason we will keep a copy of the [CHGCAR](../input-files/CHGCAR.md) file
(and the [WAVECAR](../input-files/WAVECAR.md) file as well):

    cp CHGCAR  CHGCAR.0
    cp WAVECAR WAVECAR.0

  
The information most relevant to the task at hand you will find near the
end of the [OUTCAR](../output-files/OUTCAR.md) file:
...
To add an additional spherical potential on the site of atom 1 that acts
on the *d*-manifold we specify the following:
...
Note that for [LDAUTYPE](../incar-tags/LDAUTYPE.md)`=3` the
[LDAUU](../incar-tags/LDAUU.md) and [LDAUJ](../incar-tags/LDAUJ.md) tags
specify the strength (in *eV*) of the spherical potential acting on the
spin-up and spin-down *d*-manifolds, respectively.

In the present step, we want to calculate the *non-selfconsistent*
response to this additional potential. This is done by reading the
charge density from the previous DFT ground-state calculations and by
keeping it fixed during the electronic minimization procedure:

    ICHARG       = 11

N.B.: be sure to use the charge density of the DFT groundstate
calculation:

    cp CHGCAR.0  CHGCAR
    cp WAVECAR.0 WAVECAR

After running this calculation, you will notice that due to the
additional potential, the number of *d*-electrons on atom 1 has changed
w.r.t. the DFT groundstate (check the [OUTCAR](../output-files/OUTCAR.md)
file again):
...
The change in the number of *d*-electrons on atomic site 1 is found to
be:
...
**N.B.I**: The only difference between this calculation and the previous
calculation of the *non-selfconsistent* response is that now we **do not
set** [ICHARG](../incar-tags/ICHARG.md)`=11`, *i.e*, now the charge
density may change.

**N.B.II**: To speed things up, it is a good idea to restart this
calculation from the [WAVECAR](../input-files/WAVECAR.md) file of the
previous non-selfconsistent response calculation.

After this calculation has finished, you should again inspect the number
of *d*-electrons on atomic site 1:
