...
- [1 Requirements](#Requirements)
  - [1.1 Software stacks](#Software_stacks)
    - [1.1.1 NVIDIA GPUs](#NVIDIA_GPUs)
    - [1.1.2 AMD GPUs](#AMD_GPUs)
    - [1.1.3 Intel GPUs](#Intel_GPUs)
  - [1.2 Hardware](#Hardware)
    - [1.2.1 NVIDIA GPUs](#NVIDIA_GPUs_2)
    - [1.2.2 AMD GPUs](#AMD_GPUs_2)
    - [1.2.3 Intel GPUs](#Intel_GPUs_2)
- [2 Building](#Building)
- [3 Features and limitations](#Features_and_limitations)
- [4 Running the GPU versions](#Running_the_GPU_versions)
  - [4.1 Environment variables](#Environment_variables)
    - [4.1.1 MPI](#MPI)
    - [4.1.2 OpenMP Offloading](#OpenMP_Offloading)
      - [4.1.2.1 AMD](#AMD)
      - [4.1.2.2 Intel](#Intel)
- [5 Credits](#Credits)
...
- To compile the OpenACC version of VASP you need a recent version of
  the [NVIDIA HPC-SDK](https://developer.nvidia.com/hpc-sdk) (\>=21.2).
...
- Numerical libraries: FFTW, BLAS, LAPACK, and scaLAPACK. In case you
  are using the [NVIDIA HPC-SDK](https://developer.nvidia.com/hpc-sdk)
  the only numerical library you will have to install yourself is
  [FFTW](http://www.fftw.org). The latter three (BLAS, LAPACK, and
  scaLAPACK) are shipped with the SDK. Alternatively, you can link
  against an installation of [Intel's oneAPI
  MKL](https://www.intel.com/content/www/us/en/developer/tools/oneapi/onemkl.html)
  library that provides all four.
...
- [The NVIDIA Collective Communications Library
  (NCCL)](https://developer.nvidia.com/nccl) (\>=2.7.8). This library is
  not a strict requirement but its use is highly recommended for
  performance reasons. Suitable installations of NCCL are shipped as
...
- You will also need to specify for which GPU architectures you want to
  ...
  the [CUDA GPU Compute
  Capability](https://developer.nvidia.com/cuda/gpus%7C) chart to select
  the compute capabilities (cc), e.g., `cc80,cc89` for compute
  capability 8.0 (A series datacenter GPUs) and 8.9 (RTX Ada series
  workstation GPUs). Selecting more compute capabilities will make your
  executable more portable, but will increase compile time. If you are
  compiling on a compute node directly, you can also use `ccnative`.

#### AMD GPUs
**Compiler**

- To compile the OpenMP offload version of VASP for AMD GPUs, the [HPE
  Cray compiler
  environment](https://cpe.ext.hpe.com/docs/25.03/cce/index.html) (CCE
  \>= 19.0) is needed. This compiler is only available on HPE HPC
...
- Numerical libraries: FFTW, BLAS, LAPACK, and scaLAPACK. These are all
  provided via [AMDs
  ROCm](https://www.amd.com/en/products/software/rocm/hpc.html) software
  stack, which includes hipFFT, hipBLAS, hipSolver, and more. We have
  tested ROCm versions 6.1, 6.3, and 6.4.
...
- [The ROCm Communication Collectives Library
  (RCCL)](https://rocm.docs.amd.com/projects/rccl/en/latest/). This
  library is not a strict requirement, but its use is highly recommended
  for performance reasons. Suitable installations of RCCL are shipped as
  part of ROCm.
...
- GPU drivers for AMD datacenter GPUs are baked into the OS on HPC
  ...
  AMD](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/install/quick-start.html#).

#### Intel GPUs
**Compiler**

- To compile the OpenMP offload version of VASP for AMD GPUS, the [Intel
  Fortran Compiler
  IFX](https://www.intel.com/content/www/us/en/developer/tools/oneapi/fortran-compiler.html)
  (\>=2025.3.0) is needed. It is distributed via the [oneAPI HPC
  Toolkit](https://www.intel.com/content/www/us/en/developer/tools/oneapi/hpc-toolkit.html).
...
- Numerical libraries: FFTW, BLAS, LAPACK, and scaLAPACK. These are
  provided via the [Intel Math Kernel Library
  (oneMKL)](https://www.intel.com/content/www/us/en/developer/tools/oneapi/onemkl.html),
  which is distributed in the [oneAPI Base
  Toolkit](https://www.intel.com/content/www/us/en/developer/tools/oneapi/base-toolkit.html),
  and the [oneAPI HPC
  Toolkit](https://www.intel.com/content/www/us/en/developer/tools/oneapi/hpc-toolkit.html).
- A GPU-enabled version of MPI. We have both tested
  [Intel-MPI](https://www.intel.com/content/www/us/en/developer/tools/oneapi/mpi-library.html),
  as distributed via the [oneAPI HPC
  Toolkit](https://www.intel.com/content/www/us/en/developer/tools/oneapi/hpc-toolkit.html)
  and [MPICH](https://www.mpich.org/) successfully.
...
- GPU drivers for Intel datacenter GPUs are baked into the OS on HPC
  ...
  follow the official [Intel
  guide](https://dgpu-docs.osgc.infra-host.com/driver/installation-lts2.html#installing-gpu-drivers)
  and make sure that your OS is supported.
...
## References
1.  [↑](#cite_ref-1) We are working alongside AMD to also support [AMDs
    next-gen Fortran
    compiler](https://rocm.blogs.amd.com/ecosystems-and-partners/fortran-journey/README.html),
