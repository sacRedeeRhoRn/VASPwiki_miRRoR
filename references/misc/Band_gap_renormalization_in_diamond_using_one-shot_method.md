<!-- Source: https://vasp.at/wiki/index.php/Band_gap_renormalization_in_diamond_using_one-shot_method | revid: 33385 | retrieved: 2026-06-24 -->
...
- The tags with "PHON\_" control the electron-phonon related features.
  [PHON_LMC](../incar-tags/PHON_LMC.md) enables the calculation of
  structures with random displacements (one shot or Monte Carlo) of the
  atoms according to the density matrix of a harmonic oscillator. By
  selecting [PHON_NSTRUCT](../incar-tags/PHON_NSTRUCT.md)=0 a
  one-shot configuration (ZG configuration) is obtained. The tag
  [PHON_TLIST](../incar-tags/PHON_TLIST.md) selects the temperatures
  at which the one shot calculation is obtained.
- [IBRION](../incar-tags/IBRION.md)=6 is selected to obtain the
...
information is required for a given temperature. The "Gamma" version of
VASP is used throughout this example since only a single k point is used
in the calculations.

The calculation consists of two steps:

1.  Obtain new "distorted" [POSCAR](../input-files/POSCAR.md) file which
    contains special displacements. This calculation also contains the
    band gap of the original structure.
2.  Execute simple DFT calculation for the structure containing the
    special displacements to obtain the band gap.
...
Copy the [OUTCAR](../output-files/OUTCAR.md) file to *OUTCAR.init*. It will
be later used for the band gap of the "undistorted" structure.
...
[INCAR](../input-files/INCAR.md) file related to "PHON\_" so that it looks
like the following:
...
minimum, they are averaged in the calculation of the band gap. This is
necessary since the convergence with respect to cell size is drastically
improved this way. Also, all perturbation theory calculations in the
literature evaluate the band gap the same way, which ensures the
compatibility of the different computational methods.
...
This example contains a [POSCAR](../input-files/POSCAR.md) file for a 5x5x5
cell (*POSCAR.5x5x5*). After repeating all above steps with this
[POSCAR](../input-files/POSCAR.md) file the following results should be
obtained:
...
The interested user can try to further increase the cell size, by making
a super cell from the primitive cell (*POSCAR.prim*) provided by this
...
*POSCAR* and *INCAR.init* to *INCAR*. The *INCAR* file contains
following lines which are different from the calculation of the ZPR:

    PHON_TLIST = 0.0 100.0 200.0 300.0 400.0 500.0 600.0 700.0

After running the calculations several new
[POSCAR](../input-files/POSCAR.md) files are created.
...
This script calculates the relative change of the band with respect to
temperature (that means the offset is shifted to zero). The result is
written to *gap_vs_temp.dat*. To plot the data please type the
following:
...
Go out of the directory *./TEMP_DEPENDENCE* and go to the directory
*./QUASI_HARMONIC*. For the purpose of this tutorial, the quasi-harmonic
calculations will be performed for the 4x4x4 cell but for exact
calculations, one needs to go to larger cell sizes until the results are
converged.

There are three scripts in this directory to perform the quasi-harmonic
calculations:

1.  *quasi_harm_4x4x4_diamond_create_pos_and_run_vasp.sh*
2.  *quasi_harm_4x4x4_diamond_make_energy_vs_volume_plots.sh*
3.  *quasi_harm_4x4x4_diamond_obtain_fitting.sh*

For a quasi-harmonic fit we need free energy vs. volume curves at
different temperatures. Starting from the equilibrium volume we need to
...
obtain the dynamical matrix. To run this script please set your
executable path and number of processors in the script and type:

    bash ./quasi_harm_4x4x4_diamond_create_pos_and_run_vasp.sh

This script renames all the [OUTCAR](../output-files/OUTCAR.md) files for
each volume which are needed in the next step. In that step the free
...
Now switch back to the folder *./TEMP_DEPENDENCE* and replace the
lattice parameters at each temperature in the *POSCAR.T=\** files by the
one obtained for the above fit. After that rerun the scripts
*run_temperature.sh* and *extract_temp.sh*.

To plot the newly obtained curve together with the other curves please
type:
...
cell since larger cells would be already quite time-consuming, but for
the converged 5x5x5 cell both curves should look slightly worse compared
to experiment. A discrepancy between experiment and theory is expected,
