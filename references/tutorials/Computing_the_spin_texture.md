...
$\sigma_x$, $\sigma_y$,
and $\sigma_z$
spin projections with
<a href="https://vasp.at/py4vasp/latest/index.html"
class="external text" rel="nofollow">py4vasp</a>.

|  |
|----|
| **Mind:** <a href="https://vasp.at/py4vasp/latest/index.html"
class="external text" rel="nofollow">py4vasp</a> reads the [vaspout.h5](../output-files/Vaspout.h5.md) file, which VASP writes only when it is compiled with HDF5 support. |
...
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=1"
class="mw-editsection-visualeditor"
title="Edit section: Step-by-step instructions">edit</a> \| (./index.php.md)\]

### Step 1: Compute the self-consistent ground state\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=2"
class="mw-editsection-visualeditor"
title="Edit section: Step 1: Compute the self-consistent ground state">edit</a> \| (./index.php.md)\]
...
### Step 2: Compute the band structure and identify the Rashba-split bands\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=3"
class="mw-editsection-visualeditor"
title="Edit section: Step 2: Compute the band structure and identify the Rashba-split bands">edit</a> \| (./index.php.md)\]

<figure typeof="mw:File/Thumb">
<a href="/wiki/File:Spin-texture-band-structure.png"
class="mw-file-description"><img
src="https://vasp.at/wiki/images/thumb/4/43/Spin-texture-band-structure.png/400px-Spin-texture-band-structure.png"
class="mw-file-element" decoding="async"
srcset="/wiki/images/thumb/4/43/Spin-texture-band-structure.png/600px-Spin-texture-band-structure.png 1.5x, /wiki/images/thumb/4/43/Spin-texture-band-structure.png/800px-Spin-texture-band-structure.png 2x"
width="400" height="343" /></a>
<figcaption>Example band structure along a high-symmetry path through
the time-reversal-invariant point. The spin-split pairs are degenerate
at the point and split away from it, with the band extrema displaced off
the point. This example is bulk BiTeI along
<em>H</em>-<em>A</em>-<em>L</em>.</figcaption>
</figure>
...
### Step 3: Choose the **k** plane from the crystal symmetry\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=4"
class="mw-editsection-visualeditor"
title="Edit section: Step 3: Choose the k plane from the crystal symmetry">edit</a> \| (./index.php.md)\]
...
$\sigma_x$ and $\sigma_y$): a
nonzero in-plane winding (a vortex or anti-vortex) is the Rashba
signature, and the out-of-plane component is close to zero near the
center.
...
class="mw-editsection-visualeditor"
title="Edit section: Step 4: Run the non-self-consistent calculation on the planar mesh">edit</a> \| (./index.php.md)\]
...
<tr>
<td><pre
style="margin: 0; padding: 1em; background: none; border: none; white-space: pre; overflow-x: auto; font-family: monospace;"><code>mkdir -p texture
cp INCAR POSCAR POTCAR CHGCAR texture/.
cd texture</code></pre></td>
</tr>
</tbody>
</table>
...
in Step 2. If the band extremum is at the $\Gamma$
point, no shift is needed. Otherwise, choose the shift so that a mesh
point coincides with the extremum: shift the out-of-plane component to
select the plane that contains the extremum, and shift the in-plane
components if the extremum is not at the projected zone center.
...
<a href="https://vasp.at/py4vasp/latest/index.html"
class="external text" rel="nofollow">py4vasp</a> reads.
...
### Step 5: Plot the spin texture with py4vasp\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=6"
class="mw-editsection-visualeditor"
title="Edit section: Step 5: Plot the spin texture with py4vasp">edit</a> \| (./index.php.md)\]

<figure typeof="mw:File/Thumb">
<a href="/wiki/File:Spin-texture-bitei-full-bz.png"
class="mw-file-description"><img
src="https://vasp.at/wiki/images/thumb/8/8f/Spin-texture-bitei-full-bz.png/400px-Spin-texture-bitei-full-bz.png"
class="mw-file-element" decoding="async"
srcset="/wiki/images/thumb/8/8f/Spin-texture-bitei-full-bz.png/600px-Spin-texture-bitei-full-bz.png 1.5x, /wiki/images/thumb/8/8f/Spin-texture-bitei-full-bz.png/800px-Spin-texture-bitei-full-bz.png 2x"
width="400" height="371" /></a>
<figcaption>Example of the <code>to_quiver</code> output over the full
Brillouin zone: the in-plane spin arrows wind around the band extremum,
and the threefold pattern reflects the crystal symmetry. This example is
bulk BiTeI on a plane through the top face of the Brillouin
zone.</figcaption>
</figure>

<a
href="https://vasp.at/py4vasp/latest/calculation/band/#from_path-to_quiver"
class="external text" rel="nofollow">py4vasp</a> draws the in-plane spin
as a quiver plot. Run the following in a Python notebook:
...
in the plane. Refer to the <a
href="https://vasp.at/py4vasp/latest/calculation/band/#from_path-to_quiver"
class="external text" rel="nofollow">py4vasp</a> documentation for the
details.

## Troubleshooting\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=7"
class="mw-editsection-visualeditor"
title="Edit section: Troubleshooting">edit</a> \| (./index.php.md)\]
...
### Probe explicit **k** points near the center\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=8"
class="mw-editsection-visualeditor"
title="Edit section: Probe explicit k points near the center">edit</a> \| (./index.php.md)\]
...
- **Rashba** (polar, $C_{nv}$):
  the spin is tangential, perpendicular to **k** in the plane, winding
  in a single circular sense, with $\sigma_z\approx0$ near the center. For a point along the Cartesian *x*
  axis only $\sigma_y$
  is nonzero, and along *y* only $\sigma_x$.
...
## Symmetry requirements and cell orientation\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=9"
class="mw-editsection-visualeditor"
title="Edit section: Symmetry requirements and cell orientation">edit</a> \| (./index.php.md)\]

<figure typeof="mw:File/Thumb">
<a href="/wiki/File:Rashba-dresselhaus.png"
class="mw-file-description"><img
src="https://vasp.at/wiki/images/thumb/e/e6/Rashba-dresselhaus.png/400px-Rashba-dresselhaus.png"
class="mw-file-element" decoding="async"
srcset="/wiki/images/thumb/e/e6/Rashba-dresselhaus.png/600px-Rashba-dresselhaus.png 1.5x, /wiki/images/thumb/e/e6/Rashba-dresselhaus.png/800px-Rashba-dresselhaus.png 2x"
width="400" height="205" /></a>
<figcaption>Spin textures of the Rashba (left) and Dresselhaus (right)
spin-orbit fields: arrows give the spin direction along a constant-k
circle for each effective Hamiltonian - tangential/chiral for Rashba,
mirror-symmetric for Dresselhaus</figcaption>
</figure>
...
### Which symmetry allows a spin texture\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=10"
class="mw-editsection-visualeditor"
title="Edit section: Which symmetry allows a spin texture">edit</a> \| (./index.php.md)\]
...
- **Rashba** textures require a **polar** point group, that is, one with
  a unique polar axis (the polar classes, such as
  $C_{2v}$, $C_{3v}$,
  $C_{4v}$, and $C_{6v}$).
  The Rashba spin-orbit field, and hence the in-plane winding, lies in
...
- **Dresselhaus** textures arise from bulk inversion asymmetry in
  ...
  zinc-blende ($T_d$). The
  texture is anisotropic rather than a simple tangential winding, so the
  plane is chosen to expose that pattern (for example a (001) **k**
  plane).

If the crystal is centrosymmetric, the bands are spin-degenerate and
there is no spin texture to compute.

### Locating the axis and orienting the cell\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=11"
class="mw-editsection-visualeditor"
title="Edit section: Locating the axis and orienting the cell">edit</a> \| (./index.php.md)\]
...
$C_{nv}$). A regular `n n 1` mesh samples the plane
spanned by the first two reciprocal lattice vectors, and the
<a href="https://vasp.at/py4vasp/latest/index.html"
class="external text" rel="nofollow">py4vasp</a> `sigma_x~sigma_y`
selection assumes the plane normal points along the Cartesian *z* axis.
Two points therefore need checking:
...
- **The polar axis must align with a Cartesian axis.** To use `n n 1`
  with the $\sigma_x$–$\sigma_y$
  in-plane components, the polar axis (the plane normal) must lie along
  Cartesian *z*. If it does not, either reorient the cell — rotate the
  lattice vectors so the polar axis is along *z* — and use `n n 1`, or
  choose the mesh that places the single subdivision along the matching
...
it is polar and the polar axis is the *c* axis. The plane to sample is
the $k_x$–$k_y$ plane.
In the conventional hexagonal setting the *c* axis is along Cartesian
*z*, so an `n n 1` mesh together with the `sigma_x~sigma_y` selection
works directly.

## Recommendations and advice\[<a
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=12"
class="mw-editsection-visualeditor"
...
- **Mind the k-point units.** A `Cartesian`
  ...
  $2\pi/\text{SCALE}$, where SCALE is the
  [POSCAR](../input-files/POSCAR.md) scaling factor. A value of 0.05 then
  corresponds to $0.05\cdot2\pi\approx0.31$ Å<sup>-1</sup>, which can fall far outside the
  linear Rashba region and into a regime of warping with a spurious
  $\sigma_z$. A `Reciprocal` (fractional) list avoids
  the $2\pi$
  factor, but its axes are the reciprocal lattice vectors, which are
  non-orthogonal for a hexagonal cell. There,
  $\mathbf{b}_1$ is 30° off the Cartesian *x* axis, so
  a fractional $(\delta, 0, \*)$ point is not along *x* and its tangential spin
  acquires a nonzero $\sigma_x$
  even for an ideal Rashba state. To test the "only
  $\sigma_y$ along *x*" rule, use small `Cartesian`
  coordinates so the point truly lies on the Cartesian axis.
- **Place the plane correctly in Cartesian coordinates.** In a
  `Cartesian` list the third coordinate is also scaled by
  $2\pi/\text{SCALE}$. To sit on a plane at fractional
  $k_z=1/2$ use $k_z=0.5/c$
  rather than 0.5.
...
href="/wiki/index.php?title=Computing_the_spin_texture&amp;veaction=edit&amp;section=13"
class="mw-editsection-visualeditor"
