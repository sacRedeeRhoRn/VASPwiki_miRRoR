...
<a href="https://www.python.org/" class="external text"
rel="nofollow">Python</a> functions in a pre-defined format and VASP
will recognize and execute your code while it is running. This page
...
title="Edit section: Compiling with Plugins support">edit</a> \| (./index.php.md)\]
...
## Python scripting\[<a href="/wiki/index.php?title=Plugins&amp;veaction=edit&amp;section=2"
class="mw-editsection-visualeditor"
title="Edit section: Python scripting">edit</a> \| (./index.php.md)\]
...
1.  `constants` provides a
    ...
    class="external text" rel="nofollow">dataclass</a> with quantities
    computed in VASP that must stay constant through the interface.
    Typically these include "metadata" such as the lattice vectors,
    number of grid points, etc. Some interfaces also pass in quantities
    such as the charge density as `constants`. Please check the page of
    the specific plugins' tag for more details on which quantities are
    available as constants. Alternatively, you may print the contents of
    `constants`.
...
## Exposing interfaces through entry-points (advanced)\[<a href="/wiki/index.php?title=Plugins&amp;veaction=edit&amp;section=3"
class="mw-editsection-visualeditor"
title="Edit section: Exposing interfaces through entry-points (advanced)">edit</a> \| (./index.php.md)")\]

Consider using <a
href="https://packaging.python.org/en/latest/specifications/entry-points/"
class="external text" rel="nofollow">entry points</a> in cases where you
do not want to move `vasp_plugin.py` to each new VASP calculation
directory. This is also the approach you should take, if you want to
distribute your plugin to other users. Within the group "vasp", add the
following lines to your `pyproject.toml` file for each interface you
would like to introduce new functions. For example, if you wanted to
...
## Related tags and articles\[<a href="/wiki/index.php?title=Plugins&amp;veaction=edit&amp;section=4"
class="mw-editsection-visualeditor"
