...
- [1 Theory](#Theory)
- [2 Scaling](#Scaling)
  - [2.1 Building matrix](#Building_matrix)
  - [2.2 Solving equation](#Solving_equation)
    - [2.2.1 Exact diagonalization](#Exact_diagonalization)
    - [2.2.2 Iterative solution](#Iterative_solution)
- [3 Exact diagonalization](#Exact_diagonalization_2)
- [4 Time evolution](#Time_evolution)
- [5 Lanczos algorithm](#Lanczos_algorithm)
- [6 X-ray absorption spectra](#X-ray_absorption_spectra)
- [7 Performing BSE calculations on
  GPU](#Performing_BSE_calculations_on_GPU)
- [8 Additional resources](#Additional_resources)
  - [8.1 Lectures](#Lectures)
...
\mathbf{Y}_\lambda$ and eigenvalues $\omega_\lambda$ of this equation we can find the dielectric
function including the excitonic effects.
...
where $N_k$ is the number of k-points in
the full Brillouin zone and $N_c$ and
$N_v$ are the number of conduction and
valence bands, respectively. This computation scales as

$N_k\times N_q\times (N_v\times N_v\times
N_G\times N_c\times N_c)$,

where $N_q$ is the number of q-points
and $N_G$ number of G-vectors. To
simplify it, we can estimate this computation as $N^4-N^5$ with the system size.
...
scales cubically with the matrix rank $N_{\rm
rank}^3$ or as $N^6$ with the
system size.
...
$m$. Thus, solving the equation via the
time-evolution or Lanczos algorithms scales as $N_{\rm rank}^2\times m$ or $N^4$ with the system size. The number of iterations depends on the
algorithm and the required precision, which can be selected via
...
eigenvalues $\omega_\lambda$ and
eigenvectors $X_\lambda$ of the BSE
Hamiltonian, the macroscopic dielectric which accounts for the excitonic
effects can be found

$\epsilon_M(\mathbf{q},\omega)=
1+2\lim_{\mathbf{q}\rightarrow 0}v(q)\sum_{\lambda}
\left|\sum_{c,v,k}\langle
...
- [Calculating the dielectric function and
  eigenvectors](../redirects/BSE_calculations.md)
- [Calculations beyond Tamm-Dancoff
  approximation](../redirects/BSE_calculations.md)
- [Calculations of $\varepsilon(\mathbf{q},\omega)$ for $\mathbf{q}\neq0$](/wiki/BSE_calculations#Calculations_at_finite_wavevectors "BSE calculations")
...
Fourier transform ^([\[1\]](#cite_note-sander:jcp:2017-1))
...
\delta_{\alpha\beta} - \frac{4\pi}{\Omega}\cfrac{|u_0|^2}{(\omega -
a_1 + \mathrm i\eta) - \cfrac{b_1^2}{(\omega -a_2 + \mathrm i\eta) -
\cfrac{b_2^2}{...}}},$

where $|u_0\rangle$ is an initial guess
vector computed from the dipole moments, $|u_0\rangle = \sum_{cv\mathbf{k}} \langle
c\mathbf{k}|r_\alpha|v\mathbf{k}\rangle \langle
v\mathbf{k}|r_\beta|c\mathbf{k}\rangle$. The
$a$ and $b$ coefficients are evaluated iteratively, with the iterative
algorithm stopping once the difference between $\epsilon(\omega)$ from two consecutive iterations is below a
certain threshold selected by [BSEPREC](../incar-tags/BSEPREC.md).
...
$v\to c$ transitions with non-zero
dipole moment. As such, the algorithm will ignore optically inactive
transitions and can reach convergence faster than other methods for
larger matrices.
...
[cuSOLVERMp](https://docs.nvidia.com/cuda/cusolvermp) and
[cuBLASMp](https://docs.nvidia.com/cuda/cublasmp) libraries provided
with NVHPC-SDK 24.7 or newer. To be able to use these libraries VASP has
to be compiled with HPC-X (MPI shipped with NVHPC-SDK), which can be
loaded via
...
10^{-9}$ in Gb for [`ANTIRES`](../incar-tags/ANTIRES.md)` = 0`.
In the case of exact diagonalization [`IBSE`](../incar-tags/IBSE.md)` = 2`,
...
### Lectures
- Lecture on the [Bethe-Salpeter equation and optical
  excitations](https://youtu.be/arTPHW28qNM).

### Tutorials
- Tutorial calculating optical absorption of C diamond using
  [G0W0](https://www.vasp.at/tutorials/latest/bse/part1/#BSE-e01),
  [BSE](https://www.vasp.at/tutorials/latest/bse/part1/#BSE-e02), and
  [TDDDH](https://www.vasp.at/tutorials/latest/bse/part1/#BSE-e03).
- Tutorial calculating optical absorption of LiF using
  [G0W0](https://www.vasp.at/tutorials/latest/bse/part2/#BSE-e04), the
  [independent particle approximation
  (IPA)](https://www.vasp.at/tutorials/latest/bse/part2/#BSE-e05), the
  [random phase approximation
  (RPA)](https://www.vasp.at/tutorials/latest/bse/part2/#BSE-e06), the
  [Tamm-Dancoff
  approximation](https://www.vasp.at/tutorials/latest/bse/part2/#BSE-e07)
  (TDA)\], and [beyond
  TDA](https://www.vasp.at/tutorials/latest/bse/part2/#BSE-e08).
- Tutorial on the [efficient Brillouin zone
  sampling](https://www.vasp.at/tutorials/latest/bse/part3/#BSE-e09) and
  [exciton
  analysis](https://www.vasp.at/tutorials/latest/bse/part3/#BSE-e10).
- Tutorial for calculating x-ray adsorption spectroscopy [XAS
  calculations](https://www.vasp.at/tutorials/latest/bse/part2) using
  the BSE.
...
## References
1.  [↑](#cite_ref-sander:jcp:2017_1-0) [T. Sander, G. Kresse,
