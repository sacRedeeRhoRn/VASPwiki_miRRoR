...
supercells.^([\[1\]](#cite_note-marsman:prb:02-1))
...
- [1 Generalized Bloch condition](#Generalized_Bloch_condition)
  - [1.1 Magnetization density](#Magnetization_density)
- [2 Modified Hamiltonian](#Modified_Hamiltonian)
- [3 Basis-set considerations](#Basis-set_considerations)
- [4 Symmetry](#Symmetry)
...
Bloch
condition^([\[2\]](#cite_note-sandratskii:pssb:86-2)[\[1\]](#cite_note-marsman:prb:02-1))
(set [`LNONCOLLINEAR`](../incar-tags/LNONCOLLINEAR.md)` = .TRUE.`
and [`LSPIRAL`](../incar-tags/LSPIRAL.md)` = .TRUE.`):

$\left\[ \begin{array}{c} \Psi^{\uparrow}_{\bf
k}(\bf r) \\ \Psi^{\downarrow}_{\bf k}(\bf r) \end{array} \right\] =
\left( \begin{array}{cc} e^{-i\bf q \cdot \bf R / 2} & 0\\ 0 & e^{+i\bf
q \cdot \bf R / 2} \end{array}\right) \left\[ \begin{array}{c}
\Psi^{\uparrow}_{\bf k}(\bf r-R) \\ \Psi^{\downarrow}_{\bf k}(\bf r-R)
\end{array} \right\],$

*i.e.*, from one unit cell to the next the up- and down-spinors pick up
an additional phase factor of $\exp(-i{\bf q}\cdot
{\bf R}/2)$ and $\exp(+i{\bf q}\cdot
{\bf R}/2)$, respectively, where **R** is a lattice vector of
the crystalline lattice, and **q** is the so-called spin-spiral
propagation vector.
...
${\bf m} ({\bf r} + {\bf R})= \left(
\begin{array}{c} m_x({\bf r}) \cos({\bf q} \cdot {\bf R}) - m_y({\bf r})
\sin({\bf q} \cdot {\bf R}) \\ m_x({\bf r}) \sin({\bf q} \cdot {\bf
R}) + m_y({\bf r}) \cos({\bf q} \cdot {\bf R}) \\ m_z({\bf r})
\end{array} \right)$
...
$m_z$ retains the usual cell
periodicity. This is depicted schematically below:

[![](https://vasp.at/wiki/images/thumb/1/1f/Spinspiral.png/400px-Spinspiral.png)](https://vasp.at/wiki/File:Spinspiral.png)
...
$\Psi^{\uparrow}_{\bf k}(\bf r) = \sum _{\bf G}
\rm C^{\uparrow}_{\bf k \bf G} e^{i(\bf k + \bf G -\frac{\bf
q}{2})\cdot \bf r}$

$\Psi^{\downarrow}_{\bf k}(\bf r) = \sum _{\bf
G} \rm C^{\downarrow}_{\bf k \bf G} e^{i(\bf k + \bf G +\frac{\bf
q}{2})\cdot \bf r}$
...
where in $H^{\uparrow\uparrow}$ and
$H^{\downarrow\downarrow}$ the kinetic
energy of a plane-wave component changes to:

$H^{\uparrow\uparrow}:\qquad |{\bf k} + {\bf
G}|^2 \rightarrow |{\bf k} + {\bf G} - {\bf q} /2|^2$

$H^{\downarrow\downarrow}:\qquad |{\bf k} + {\bf
G}|^2 \rightarrow |{\bf k} + {\bf G} + {\bf q} /2|^2$
...
calculation^([\[3\]](#cite_note-hobbs:prb:00-3)) of the primitive cell.

## Basis-set considerations
Because the two spinor components are shifted in reciprocal space by
$\pm{\bf q}/2$, the plane-wave cutoff
must be chosen carefully. The cutoff of the basis set of the individual
spinor components is specified by means of the
[ENINI](../incar-tags/ENINI.md) tag, while [ENMAX](../redirects/ENMAX.md)
must be chosen large enough that the plane-wave components of both
spinors have a kinetic energy below [ENMAX](../redirects/ENMAX.md). This
is the case when

$\mathtt{ENMAX} \geq \frac{\hbar^2}{2m}\left(
G_{\rm ini} + |q| \right)^2, \qquad G_{\rm
ini}=\sqrt{\frac{2m}{\hbar^2}\mathtt{ENINI}}.$

In practice it is more than sufficient to set
[ENMAX](../redirects/ENMAX.md) to [ENINI](../incar-tags/ENINI.md) + 100 eV.
The practical settings, and the runtime warning VASP prints when
[ENMAX](../redirects/ENMAX.md) is too small, are described in [Spin-spiral
calculations](../tutorials/Spin-spiral_calculations.md).
...
[ENMAX](../redirects/ENMAX.md), [ISYM](../incar-tags/ISYM.md),
[MAGMOM](../incar-tags/MAGMOM.md)

## References
1.  ↑ ^([a](#cite_ref-marsman:prb:02_1-0))
    ^([b](#cite_ref-marsman:prb:02_1-1)) [M. Marsman and J. Hafner,
    *Broken symmetries in the crystalline and magnetic structures of
    γ-iron*, Phys. Rev. B **66**, 224409
    (2002).](https://doi.org/10.1103/PhysRevB.66.224409)
2.  [↑](#cite_ref-sandratskii:pssb:86_2-0) [L. M. Sandratskii, *Energy
    band structure calculations for crystals with spiral magnetic
    structure*, Phys. Status Solidi B **136**, 167
    (1986).](https://doi.org/10.1002/pssb.2221360119)
3.  [↑](#cite_ref-hobbs:prb:00_3-0) [Hobbs, D., G. Kresse, and J.
