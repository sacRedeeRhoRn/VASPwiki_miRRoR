...
## Introduction\[<a
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=1"
class="mw-editsection-visualeditor"
title="Edit section: Introduction">edit</a> \| (./index.php.md)\]

Under the presence of a small, finite external electric field,
$\mathcal{E_\alpha}$, the changes in the total energy
of the system due to $\mathcal{E_\alpha}$ can be computed from the polarization at first order,
and at second order in $\mathcal{E_\alpha}$ from the dielectric susceptibility, Born-effective
charges, and piezoelectric tensor.
...
$|\psi^{\mathcal{E}_\alpha}_\lambda\rangle = |\psi\rangle + \lambda
|\partial_{\mathcal{E}_\alpha}\psi\rangle,$

with $\lambda$
being the vanishing strength of the field, and the static dielectric
tensor

$\varepsilon^{\infty}(\hat{\mathbf q},0)= 1 - \frac{8 \pi e^2}{\Omega}
\lim _{\mathbf{q} \rightarrow 0}\frac{1}{\mathbf q^2} \sum_{c, v,
\mathbf{k}} 2 w_{\mathbf{k}} \left|\langle u_{c \mathbf{k+q}}|u_{v
\mathbf{k}}\rangle\right|^2,$
...
$u_{n \mathbf{k}+\mathbf{q}}=u_{n \mathbf{k}}+\mathbf{q} \cdot
\nabla_{\mathbf{k}} u_{n \mathbf{k}}+O\left(\mathbf{q}^2\right).$

So, both the derivative with respect to the crystal momentum **k** and
the finite electric field $\mathcal{E_\alpha}$ are required. Fortunately both can be obtained from a
system of coupled equations called the Sternheimer equations.

## Sternheimer equations\[<a
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=2"
class="mw-editsection-visualeditor"
title="Edit section: Sternheimer equations">edit</a> \| (./index.php.md)\]
...
$e_{n\mathbf{k}}$ are the KS eigenpairs.
...
Here $\Delta
H_{\text{SCF}}(\mathbf{k}) |\tilde{u}_{n\mathbf{k}}\rangle$ is the change in the microscopic cell periodic part of
the Hamiltonian that comes from the change in wave functions
(self-consistent field effects). These are computed at first order only,
thus ensuring that local field effects are included directly in the
evaluation of the dielectric response.

The derivative with respect to $\mathbf{k}$
in the second equation enters via the polarization vector,
$\vec\beta_{n\mathbf k}$, which in the [PAW
formalism](../methods/Projector-augmented-wave_formalism.md)
is given by

$|\vec{\beta}_{n\mathbf{k}}\rangle= \left( 1+\sum_{ij}
...
where $Q_{ij}$ and
$\vec\tau_{ij}$ are the norm and the dipole moments of
the one-center charge densities, each respectively expressed as

$Q_{ij} = \int_{\Omega_\text{PAW}} \left\[ \phi_i(\mathbf{r})
\phi_j(\mathbf{r}) - \tilde{\phi}_i(\mathbf{r})
\tilde{\phi}_j(\mathbf{r}) \right\] d^3 \mathbf{r}$

and

$\vec{\tau}_{ij} = \int_{\Omega_\text{PAW}} (\mathbf{r}-\mathbf{R}_i)
\left\[ \phi_i(\mathbf{r}) \phi_j(\mathbf{r}) -
\tilde{\phi}_i(\mathbf{r}) \tilde{\phi}_j(\mathbf{r}) \right\] d^3
\mathbf{r}.$

For norm-conserving pseudopotentials the expression for
$|\vec{\beta}_{n\mathbf{k}}\rangle$ is reduced to
...
## Computation of physical quantities\[<a
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=3"
class="mw-editsection-visualeditor"
title="Edit section: Computation of physical quantities">edit</a> \| (./index.php.md)\]
...
$\epsilon^\infty(\hat{\mathbf{q}},0) = 1-\frac{8\pi e^2}{\Omega}
\sum_{v\mathbf{k}} 2 w_\mathbf{k} \langle
...
on each atom, $a$, for a
given set of perturbed KS orbitals it is also possible to obtain the
Born effective charges using

$Z^a_{ij}=\frac{\Omega}{e}\frac{\partial P_i}{\partial u^a_j}
=\frac{1}{e}\frac{\partial F^a_j}{\partial \mathcal{E}_i}
\approx\frac{1}{e}\frac{ \left(
...
Finally, the ion-clamped piezoelectric tensor
$\overline{e}_{ij}$ can be computed from the strain
tensor, $\mathbf{\sigma}\[\psi_{n\mathbf{k}}\]$, using

$\overline{e}_{ij} =-\frac{\partial \sigma_j}{\partial \mathcal{E}_i}
\approx -\frac{ \left( \sigma\[\\\psi^{\mathcal{E}_i}_{\lambda/2}\\\]-
...
### Ionic contributions\[<a
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=4"
class="mw-editsection-visualeditor"
title="Edit section: Ionic contributions">edit</a> \| (./index.php.md)\]

#### Piezoelectric tensor\[<a
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=5"
class="mw-editsection-visualeditor"
title="Edit section: Piezoelectric tensor">edit</a> \| (./index.php.md)\]
...
where $\Phi_{mn}$
and $\Xi_{nj}$
are the force constants and force response to the internal strain
tensors, respectively. The second term on the right-hand side will be
written to the [OUTCAR](../output-files/OUTCAR.md) in
...
#### Dielectric tensor\[<a
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=6"
class="mw-editsection-visualeditor"
...
$\chi_{\alpha\beta} = \overline{\chi_{\alpha\beta}} +
...
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=7"
class="mw-editsection-visualeditor"
...
## References\[<a
href="/wiki/index.php?title=Electric_field_response_from_density-functional-perturbation_theory&amp;veaction=edit&amp;section=8"
class="mw-editsection-visualeditor"
