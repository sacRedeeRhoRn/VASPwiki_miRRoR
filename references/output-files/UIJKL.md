...
# UIJKL
The UIJKL file stores the effectively screened Coulomb integrals at the
unit-cell origin (**R** = 0):

$U_{ijkl}^{\sigma\sigma'} = \int {\rm d}{\bf
r}\int {\rm d}{\bf r}' w_{i}^{\*\sigma}({\bf r}) w_{j}^{\sigma}({\bf
r}) U({\bf r},{\bf r}',\omega) w_{k}^{\*\sigma'}({\bf r}')
w_{l}^{\sigma'}({\bf r}')$

evaluated at zero frequency ($\omega=0$). The columns I, J, K, L are the Wannier function indices; RE
and IM are the real and imaginary parts. The format is as follows:
