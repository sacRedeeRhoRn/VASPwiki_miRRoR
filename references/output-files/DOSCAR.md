<!-- Source: https://vasp.at/wiki/index.php/DOSCAR | revid: 34299 | retrieved: 2026-06-24 -->
...
which is followed by [NEDOS](../incar-tags/NEDOS.md) lines with the columns
...
In the case of colinear calculations with
[ISPIN](../incar-tags/ISPIN.md)=1, the format for l-decomposed calculations
for each ion is
...
For spin-polarized systems [ISPIN](../incar-tags/ISPIN.md) = 2 additional
columns are printed for each spin channel

    energy s-DOS(up) s-DOS(down) p-DOS(up) p-DOS(dwn) d-DOS(up) d-DOS(dwn)

and correspondingly if lm-decomposed densities are selected with
[LORBIT](../incar-tags/LORBIT.md).

For non-collinear calculations, information on the individual spinor
components is available only for the site projected density of states:
...
is not equal 1 (contrary to previous releases).

- vasp.5 needs no specification of [NPAR](../incar-tags/NPAR.md)

|  |
|----|
| **Mind:** For relaxations, the DOSCAR is usually useless. If you want to get an accurate DOS for the final configuration, first copy [CONTCAR](CONTCAR.md) to [POSCAR](../input-files/POSCAR.md) and continue with one static ([`ISTART`](../incar-tags/ISTART.md)` = 1`; [`NSW`](../incar-tags/NSW.md)` = 0`) calculation. Mind that for NSW\>1 the final DOS is averaged of all ionic steps. See [KBLOCK](../incar-tags/KBLOCK.md) for details. |
