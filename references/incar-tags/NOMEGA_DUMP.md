...
Description: NOMEGA_DUMP selects the imaginary frequency point of
screened potential in [low-scaling GW
...
NOMEGA_DUMP selects the imaginary frequency point of the screened
Coulomb kernel that is written to
[WFULLxxxx.tmp](../input-files/WFULLxxxx.tmp.md) in [low-scaling GW
calculations](../methods/Practical_guide_to_GW_calculations.md).
If set to 0, [WFULLxxxx.tmp](../input-files/WFULLxxxx.tmp.md)
contains the screened Coulomb interaction W at $\omega=0$. For positive values, these files contain the
