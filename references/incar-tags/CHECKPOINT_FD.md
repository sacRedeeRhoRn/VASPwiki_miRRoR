...
[Phonons](../redirects/Phonons.md) can be calculated using [finite
...
frequencies are calculated. Using CHECKPOINT_FD (with
[`IBRION`](IBRION.md)` = 6`), it is possible to [restart a
calculation that
crashed](../tutorials/Construction-Restarting_finite_differences_calculations.md)
from the last displacement, or [split the calculation into individual
displacements](../tutorials/Construction-Restarting_finite_differences_calculations.md).
The displacements are written to the
[vaspcheckfd.h5](../input-files/Vaspcheckfd.h5.md) file.
...
- [1 CHECKPOINT_FD = CONTINUE](#CHECKPOINT_FD_=_CONTINUE)
- [2 CHECKPOINT_FD = RESET (default)](#CHECKPOINT_FD_=_RESET_(default))
- [3 CHECKPOINT_FD = NONE](#CHECKPOINT_FD_=_NONE)
- [4 CHECKPOINT_FD = PREPARE](#CHECKPOINT_FD_=_PREPARE)
- [5 CHECKPOINT_FD = SINGLE](#CHECKPOINT_FD_=_SINGLE)
...
`CHECKPOINT_FD`` = CONTINUE` will perform a single SCF at the
equilibrium structure and read the displacements from
...
after performing one [electronic
minimization](../redirects/Electronic_minimization.md)
to obtain the SCF solution of the unperturbed structure.
...
produced by `CHECKPOINT_FD`` = PREPARE`. This also produces a
