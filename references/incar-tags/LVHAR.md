...
$V_{\text{ionic}}(\mathbf{r}) +V_{\text{hartree}}(\mathbf{r})$ (in eV) is written to the
...
where $V_{\text{ionic}}(\mathbf{r})$ is the ionic potential as mimicked by the
pseudopotentials and $V_{\text{hartree}}(\mathbf{r})$ is the Hartree
potential.
...
$V_{\text{ionic}}(\mathbf{r}) +V_{\text{hartree}}(\mathbf{r})$ in VASP \< 5.2.12.

## Related tags and articles\[<a href="/wiki/index.php?title=LVHAR&amp;veaction=edit&amp;section=1"
