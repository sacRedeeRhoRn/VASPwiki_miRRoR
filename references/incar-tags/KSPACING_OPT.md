...
KSPACING_OPT is used to define
the **k**-point mesh of the
...
KSPACING_OPT is the smallest
allowed spacing between **k** points in units of
$\AA^{-1}$. The number of **k** points increases when
the spacing is decreased. The number of **k** points in the direction of
the first, second and third reciprocal lattice vector is determined by
$N_i= \mathrm{max}(1, \mathrm{ceiling}( | \mathbf{b}_i| 2\pi /
\mathrm{KSPACING\\OPT} ))$, where
...
$\mathbf{b}_i \mathbf{a}_j = \delta_{ij}$. The
generated grid is centered at the $\Gamma$ point
...
href="/wiki/index.php?title=KSPACING_OPT&amp;veaction=edit&amp;section=1"
