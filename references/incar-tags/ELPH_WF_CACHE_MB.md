...
where $\mathbf{k}' = \mathbf{k} +
\mathbf{q}$. Because k-points are distributed across
MPI ranks, the wavefunction $\psi_{n\mathbf{k}}$ needed to form the bra may reside on a different rank
than the one computing the matrix element.
ELPH_WF_CACHE_MB sets the
maximum memory (in megabytes) used to cache these remotely fetched
wavefunctions locally, avoiding repeated inter-rank MPI communication.
...
href="/wiki/index.php?title=ELPH_WF_CACHE_MB&amp;veaction=edit&amp;section=1"
