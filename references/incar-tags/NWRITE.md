<!-- Source: https://vasp.at/wiki/index.php/NWRITE | revid: 32813 | retrieved: 2026-06-24 -->
<!-- © VASP wiki contributors. Licensed under GNU Free Documentation License 1.2 (GFDL 1.2). -->

# NWRITE
NWRITE = 0 \| 1 \| 2 \| 3 \| 4  
Default: **NWRITE** = 2 

Description: This tag determines how much will be written to the file
[OUTCAR](../output-files/OUTCAR.md) ('verbosity tag').

------------------------------------------------------------------------

The options for NWRITE are given in detail as

|  |  |  |  |  |
|:--:|:--:|:--:|:--:|:--:|
| Feature | NWRITE = 0 | NWRITE = 1 | NWRITE = 2 | NWRITE = 3 |
| Contributions to electronic energy at each electronic iteration | f | f | e | e |
| Convergence information | f | f | e | e |
| Eigenvalues | f+l | i | i | e |
| DOS + charge density | f+l | i | i | e |
| Total energy and electronic contributions | i | i | i | i |
| Stress | i | i | i | i |
| Basis vectors | f+l | i | i | i |
| Forces | f+l | i | i | i |
| Lattice and space group information for [ISYM](ISYM.md)\>0 | f | f | f | f |
| Symmetry operations for [ISYM](ISYM.md)\>0 |  |  |  | f |
| Timing information |  |  | X | X |

where the following abbreviations have been used

|      |                           |
|:----:|:-------------------------:|
| Code |          Meaning          |
| f+l  | first and last ionic step |
|  f   |     first ionic step      |
|  i   |      each ionic step      |
|  e   |   each electronic step    |
|  X   |      when applicable      |

  

|  |
|----|
| **Tip:** For long molecular-dynamics runs, use `NWRITE`` = 0` or `NWRITE`` = 1`. For short runs use `NWRITE`` = 2`. `NWRITE`` = 3` might give information if something goes wrong. |

|                                                      |
|------------------------------------------------------|
| **Important:** `NWRITE`` = 4` is for debugging only. |

## Related tags and articles
[OUTCAR](../output-files/OUTCAR.md), [IALGO](IALGO.md),
[IBRION](IBRION.md), [MDALGO](MDALGO.md),
[ISIF](ISIF.md), [ISYM](ISYM.md),
[EDIFF](EDIFF.md), [EDIFFG](EDIFFG.md),
[Troubleshooting electronic
convergence](../tutorials/Troubleshooting_electronic_convergence.md)

[Examples that use this
tag](https://vasp.at/wiki/index.php/Special-Search/-NWRITE-_incategory-Examples)
