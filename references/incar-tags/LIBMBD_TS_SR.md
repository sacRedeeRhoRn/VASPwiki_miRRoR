...
Description: LIBMBD_TS_SR sets the value of the scaling factor
$s_R$ in the Tkatchenko-Scheffler
...
LIBMBD_TS_SR allows to choose the value of the scaling factor
$s_R$ in the Tkatchenko-Scheffler
method^([\[1\]](#cite_note-tkatchenko:prl:09-1)) as implemented in the
library libMBD of many-body dispersion
methods^([\[2\]](#cite_note-libmbd_1-2)[\[3\]](#cite_note-libmbd_2-3)[\[4\]](#cite_note-hermann:jcp:2023-4)).
The value is internally passed to the libMBD input **ts_sr** described
at the page ^([\[5\]](#cite_note-libmbd_input-5)). LIBMBD_TS_SR is
similar to the [VDW_SR](VDW_SR.md) tag that is used for the
VASP implementation of the Tkatchenko-Scheffler method.
