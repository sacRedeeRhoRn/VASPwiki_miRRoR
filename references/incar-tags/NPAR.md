...
Description: NPAR determines the number of bands that are treated in
parallel. This is a legacy tag; use [NCORE](NCORE.md)
instead.
...
NPAR sets the number of band groups; [NCORE](NCORE.md) sets
the size of each band group. They are strict inverses:

$\text{NPAR} \times \text{NCORE} = \text{available
ranks}$

The default (NPAR = available ranks) is equivalent to
[`NCORE`](NCORE.md)` = 1`: each band is handled by a single
rank.
