...
number $L_\max$ of spherical harmonics
used to expand atomic distributions within the machine learning force
field method.
...
where $g\left(\mathbf{r}\right)$ is an
approximation of the delta function, is expanded in terms of radial
basis functions $\chi_{nl}(r)$ and
spherical harmonics $Y_{lm}\left(\hat{\mathbf{r}}\right)$ (see [this
...
The tag ML_LMAX2 specifies the maximum angular momentum quantum number
$L_\max$ of spherical harmonics used in
this expansion.
...
of ML_LMAX2=3 is perfectly safe for most applications. If you need to
improve the accuracy, you might also try to increase it to ML_LMAX2=4.
If the angular filter is switched off, we recommend to use a value of
ML_LMAX2=6 (this is then the default value).
