...
file. Setting `ELPH_POT_GENERATE`` = True` provides a way to generate
the [phelel_params.hdf5](../input-files/Phelel_params.hdf5.md)
file directly in VASP by [computing the electron-phonon
...
`ELPH_POT_GENERATE`` = True`, it is necessary to set
[`IBRION`](IBRION.md)` = 6` to activate the
...
When `ELPH_POT_GENERATE`` = True`, VASP will additionally produce the
