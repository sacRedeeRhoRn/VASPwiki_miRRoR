...
Description: when LAECHG=.TRUE. the *all-electron* charge density will
be reconstructed explicitly and written to files.

------------------------------------------------------------------------

If LAECHG=.TRUE. is set VASP will reconstruct the *all-electron* charge
density on the so-called "fine" FFT-grid. This "fine" FFT-grid consists
of
...
In fact, for LAECHG=.TRUE., VASP will reconstruct three distinct
*all-electron* densities:
...
[AECCAR0](https://vasp.at/wiki/index.php/index.php)"),
[AECCAR1](https://vasp.at/wiki/index.php/index.php)"),
and
[AECCAR2](https://vasp.at/wiki/index.php/index.php)"),
...
$\\ \widetilde{\psi}_{n\mathbf{k}} \\$
is given by:

$n(\mathbf{r})= \sum_{n{\mathbf{k}}}
f_{n{\mathbf{k}}} \langle \widetilde{\psi}_{n\mathbf{k}}|
\mathbf{r}\rangle\langle \mathbf{r}| \widetilde{\psi}_{n\mathbf{k}}
\rangle + \sum_{\alpha, \beta} ( \phi^\ast_\alpha(\mathbf{r})
\phi_\beta (\mathbf{r}) - \widetilde{\phi}^\ast_\alpha(\mathbf{r})
\widetilde{\phi}_\beta (\mathbf{r}) ) \sum_{n{\mathbf{k}}}
f_{n{\mathbf{k}}}
...
For LAECHG=.TRUE., however, this reconstruction is exactly the thing
that is done.
