...
WRT_DENSITY can select one or multiple densities on the real-space grid
in the unit cell to be written, e.g.,
...
[XC functional](../redirects/XC_functional.md) on the plane-wave
grid to [vaspout.h5](../output-files/Vaspout.h5.md) in VASP units. That
is $\AA^{-3}$ for charge and
$\mu_B$ for the magnetization.
Correspondingly, $\AA^{-4}$ for the
gradient, etc.
...
specified with WRT_DENSITY and the system it yields
...
([ISPIN](ISPIN.md)=2) or $m_1$, $m_2$ and
$m_3$ in the 2nd, 3rd and 4th component
