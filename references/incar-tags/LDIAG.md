...
<a href="https://en.wikipedia.org/wiki/Rayleigh%E2%80%93Ritz_method"
class="external text" rel="nofollow">Rayleigh–Ritz method</a>. This step
increases the convergence rate and thus is expedient in most cases.
Furthermore, the subspace diagonalization sorts the orbital/eigenvalues
in ascending order.
...
to switch off the subspace diagonalization by specifying
LDIAG = `.FALSE.` in the
[INCAR](../input-files/INCAR.md) file. Specifically, for
[ALGO](ALGO.md) = `VeryFast`,
LDIAG = `.FALSE.` changes from
...
diagonalization is simply skipped if
LDIAG = `.FALSE.` is set.
Generally using LDIAG =
`.FALSE.` is only advised, if one wants to maintain a certain orbital
order, for instance, when reading the orbitals from an existing
...
Other combinations using LDIAG
= `.FALSE.` are likely to yield undesirable results.

|  |
|----|
| **Warning:** [ALGO](ALGO.md) = `VeryFast` is not supported for hybrid functionals. |

## Related tags and articles\[<a href="/wiki/index.php?title=LDIAG&amp;veaction=edit&amp;section=1"
