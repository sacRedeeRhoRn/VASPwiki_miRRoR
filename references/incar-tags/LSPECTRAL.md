...
If LSPECTRAL = .TRUE. is set, the imaginary part of the independent
particle polarizability $\chi_{\mathbf{q}}^0
(\mathbf{G}, \mathbf{G}', \omega)$ is calculated first, and
afterwards the full independent particle polarizability is determined
using a Kramers-Kronig (or Hilbert) transform. This reduces the
computational workload by almost a factor
...
is strongly recommended to set LSPECTRAL = .TRUE., except if memory
requirements are too excessive.
