...
Kohn-Sham orbitals onto the localized orbitals specified with the
[LOCPROJ](LOCPROJ.md) tag. This file is built specifically
to be human-readable but contains the same information as the
[LOCPROJ](LOCPROJ.md) file.
...
$\langle
\beta_{lm}^{\alpha}|S|\psi_{n\mathbf{k}}\rangle$ onto the
different angular characters of the radial function. To find a list of
the possible angular character, go to [LOCPROJ](LOCPROJ.md)
and see the table in **\<functions-Ylm-specs\>**.
