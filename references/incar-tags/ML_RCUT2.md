...
Description: This flag sets the cutoff radius
$R_\text{cut}$ for the angular descriptor
$\rho^{(3)}_i(r)$ in the machine learning force field
method. The unit of the cut-off radius is $\AA$.
...
and $g\left(\mathbf{r}\right)$ is an approximation of the delta function. A basis set
expansion of $\rho^{(3)}_i(r)$ yields the expansion coefficients
$p_{n\nu l}^{i}$ which are used in practice to describe
the atomic environment (see [this
section](../methods/Machine_learning_force_field-_Theory.md)
for details). The tag ML_RCUT2
sets the cutoff radius $R_\text{cut}$ at which the cutoff function
$f_{\mathrm{cut}}\left(r_{ij}\right)$ decays to zero.
...
ML_RCUT2 is set to values around 4 $\AA$ (that
is, less training data are required to achieve a specific accuracy),
wheres for materials with very long bonds, a larger value around 6
$\AA$ can improve the accuracy,

## Related tags and articles\[<a href="/wiki/index.php?title=ML_RCUT2&amp;veaction=edit&amp;section=1"
