...
$\xi_{0\mu}(t+\Delta t) = \xi_{0\mu}(t) +
\dot{\xi}_{\mu}(q)\Delta t, \\$

where $\Delta t$ is the time step used
in MD ([POTIM](POTIM.md)). The rate $\dot{\xi}_{\mu}$ can be defined via the parameter SPRING_V0
and its units are $uc/fs$, where
$uc$ corresponds to the units of the
coordinate the potential acts upon (e.g., ${\AA}$ for coordinates with `flag` R, $rad.$ for coordinates with `flag` A, dimensionless for coordinates
with `flag` W, etc...). The number of items defined via SPRING_V0 must
be equal to $M_8$, otherwise the
calculation terminates with an error message.
