...
1.  The ML_GRACE_MODEL tag may be used to provide just the model name.
    In this case the model is assumed to reside in the default
    `tensorpotential` download directory `~/.cache/grace`.
2.  If a full path is provided in the ML_GRACE_MODEL tag then the model
    at this exact location is used.
