...
$V_{\text{LOCPOT}}(\mathbf{r})$ (in eV)
is written to the [LOCPOT](../output-files/LOCPOT.md) file.
...
the ionic potential, the second term is the Hartree potential, and
$V_{\text{xc}}(\mathbf{r})$ is the
(semi-)local exchange-correlation potential.

If LVTOT=.TRUE., the $V_{\text{LOCPOT}}(\mathbf{r})$ is written to the
