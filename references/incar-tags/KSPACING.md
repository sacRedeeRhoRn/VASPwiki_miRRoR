...
KSPACING is the smallest allowed spacing between **k** points in units
of $\AA^{-1}$. The number of **k**
points increases when the spacing is decreased.

The number of **k** points in the direction of the first, second and
third reciprocal lattice vector is determined by $N_i= \mathrm{max}(1, \mathrm{ceiling}( | \mathbf{b}_i| 2\pi /
\mathrm{KSPACING} ))$, where $\mathrm{ceiling}( x )$ returns the least integer that is equal
or larger than $x$. Here,
$\mathbf{b}_i$ are the reciprocal
lattice vectors $\mathbf{b}_i \mathbf{a}_j =
\delta_{ij}$.

The generated grid is centered at the $\Gamma$ point if [`KGAMMA`](KGAMMA.md)` = T` (default),
i.e., includes the $\Gamma$ point. For
