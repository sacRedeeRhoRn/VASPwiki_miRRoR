...
centered at the $\Gamma$ point.

------------------------------------------------------------------------

If `KGAMMA`` = .TRUE.` (default), the **k**-point mesh is centered at
the $\Gamma$ point, i.e., it always
includes the $\Gamma$ point. If
`KGAMMA`` = .FALSE.`, the mesh is shifted away from the
$\Gamma$ point as in a Monkhorst-Pack
grid. The number of **k** points on the mesh is controlled by
[KSPACING](KSPACING.md) and
