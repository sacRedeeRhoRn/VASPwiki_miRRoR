...
\mathbf{q}}\rangle$. ELPH_NBANDS_SUM specifies the maximum
number of bands, $N_{\text{b}}$, such
that $m$ runs from
$1 \ldots N_{\text{b}}$.

Multiple values can be specified for ELPH_NBANDS_SUM, in which case the
self-energy is computed once for each value. The results are reported in
separate groups inside the [vaspout.h5](../output-files/Vaspout.h5.md)
file:
