...
maximum angular momentum quantum number *L* for the augmentation of
charge densities in Hartree-Fock type routines.
...
plane wave grid is controlled by
LMAXFOCK.
...
f_{n\mathbf{k}} f_{m\mathbf{q}} \times \int\int d^3\mathbf{r}
d^3\mathbf{r}'
...
the overlap density $\psi_{n\mathbf{k}}^{\*}(\mathbf{r})\psi_{m\mathbf{q}}(\mathbf{r})$ between two Bloch orbitals needs to be calculated on
the plane wave grid (see [PAW
method](../methods/Projector-augmented-wave_formalism.md)).
The tag LMAXFOCK controls up
to which *L* quantum number, the compensation charge
$\widehat{n}(\mathbf{r})$ is calculated on the plane
wave grid (compared [PAW
method](../methods/Projector-augmented-wave_formalism.md)
compensation charge):
...
LMAXFOCK, VASP uses a small
trick: the contributions from the Hartree-Fock one-center terms are
evaluated for the pseudo orbitals also only up to
*L*=LMAXFOCK, whereas the
one-center terms for the exact all-electron orbitals are evaluated up to
the maximum required *L* (twice the angular quantum number of the
partial wave with the highest *l*). The default is
LMAXFOCK=4, and it might be
necessary to increase this parameter, if the system contains
...
results are already reasonably converged using the default
LMAXFOCK=4.

To be compatible w.r.t. old releases, VASP also reads the flag
[HFLMAX](HFLMAX.md) to the same effect as
LMAXFOCK.

## Related tags and articles\[<a href="/wiki/index.php?title=LMAXFOCK&amp;veaction=edit&amp;section=1"
