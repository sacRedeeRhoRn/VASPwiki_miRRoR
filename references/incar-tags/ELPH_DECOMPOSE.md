...
contributions^([\[1\]](#cite_note-engel:prb:2022-1)). Each contribution
can be included by specifying the associated letter in ELPH_DECOMPOSE.
We suggest two different combinations to define matrix elements:

`ELPH_DECOMPOSE`` = VDPR` - "All-electron" (AE) matrix element^([\[1\]](#cite_note-engel:prb:2022-1)[\[2\]](#cite_note-chaput:prb:2019-2))  
...
`ELPH_DECOMPOSE`` = VDQ` - "Pseudo" (PS) matrix element^([\[1\]](#cite_note-engel:prb:2022-1)[\[3\]](#cite_note-engel:prb:2020-3))  
...
renormalization](../tutorials/Bandgap_renormalization_due_to_electron-phonon_coupling.md)^([\[1\]](#cite_note-engel:prb:2022-1)).
...
\equiv \langle \tilde{\psi}_{m \mathbf{k}'} | \frac{\partial
\tilde{v}}{\partial u_{a}} | \tilde{\psi}_{n \mathbf{k}} \rangle$
...
\equiv \sum_{bij} \langle \tilde{\psi}_{m \mathbf{k}'} |
\tilde{p}_{b i} \rangle \frac{\partial D_{b, ij}}{\partial u_{a}}
\langle \tilde{p}_{b j} | \tilde{\psi}_{n \mathbf{k}} \rangle$
...
parameters, we refer to Ref. ^([\[4\]](#cite_note-kresse:prb:99-4)).

P - Derivative of PAW projectors, $|\tilde{p}_{ai}\rangle$  
$\begin{split} g^{(\text{P})}_{m \mathbf{k}', n
\mathbf{k}, a} & \equiv \sum_{ij} \langle \tilde{\psi}_{m \mathbf{k}'}
| \frac{\partial \tilde{p}_{a i}}{\partial u_{a}} \rangle ( D_{a,
ij} - \varepsilon_{n \mathbf{k}} Q_{a, ij} ) \langle \tilde{p}_{a j}
...
\equiv (\varepsilon_{n \mathbf{k}} - \varepsilon_{m \mathbf{k}'})
\sum_{ij} \langle \tilde{\psi}_{m \mathbf{k}'} | \tilde{p}_{a i}
\rangle R_{a, ij} \langle \tilde{p}_{a j} | \tilde{\psi}_{n
\mathbf{k}} \rangle$

with $R_{a, ij} \equiv \langle \phi_{a i} |
\frac{\partial \phi_{a j}}{\partial u_{a}} \rangle - \langle
\tilde{\phi}_{a i} | \frac{\partial \tilde{\phi}_{a j}}{\partial
u_{a}} \rangle$

Q - Derivative of PAW projectors, $|\tilde{p}_{ai}\rangle$ (different eigenvalues)  
$\begin{split} g^{(\text{Q})}_{m \mathbf{k}', n
\mathbf{k}, a} & \equiv \sum_{ij} \langle \tilde{\psi}_{m \mathbf{k}'}
| \frac{\partial \tilde{p}_{a i}}{\partial u_{a}} \rangle ( D_{a,
ij} - \varepsilon_{n \mathbf{k}} Q_{a, ij} ) \langle \tilde{p}_{a j}
| \tilde{\psi}_{n \mathbf{k}} \rangle \\ & + \sum_{ij} \langle
...
eigenvalues. While $g^{(\text{P})}$ uses
the eigenvalues of both the initial and final state (so
$\varepsilon_{n \mathbf{k}}$ and
$\varepsilon_{m \mathbf{k}'}$),
$g^{(\text{Q})}$ only uses the
eigenvalues of the initial state ($\varepsilon_{n
\mathbf{k}}$).
...
## References
1.  ↑ ^([a](#cite_ref-engel:prb:2022_1-0))
    ^([b](#cite_ref-engel:prb:2022_1-1))
    ^([c](#cite_ref-engel:prb:2022_1-2))
    ^([d](#cite_ref-engel:prb:2022_1-3)) [M. Engel, H. Miranda, L.
    Chaput, A. Togo, C. Verdi, M. Marsman, and G. Kresse, *Zero-point
    renormalization of the band gap of semiconductors and insulators
    using the projector augmented wave method*, Phys. Rev. B **106**,
    094316
    (2022).](https://link.aps.org/doi/10.1103/PhysRevB.106.094316)
2.  [↑](#cite_ref-chaput:prb:2019_2-0) [L. Chaput, A. Togo, and I.
    Tanaka, *Finite-displacement computation of the electron-phonon
    interaction within the projector augmented-wave method*, Phys. Rev.
    B **100**, 174304
    (2019).](https://doi.org/10.1103/PhysRevB.100.174304)
3.  [↑](#cite_ref-engel:prb:2020_3-0) [M. Engel, M. Marsman, C.
    Franchini, and G. Kresse, *Electron-phonon interactions using the
    projector augmented-wave method and Wannier functions*, Phys. Rev. B
    **101**, 184302
    (2020).](https://doi.org/10.1103/PhysRevB.101.184302)
4.  [↑](#cite_ref-kresse:prb:99_4-0) [I. G. Kresse and D. Joubert, Phys.
    Rev. B **59**, 1758
