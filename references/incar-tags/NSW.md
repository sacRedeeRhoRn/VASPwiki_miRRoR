...
NSW gives the number of steps in all [molecular
dynamics](../redirects/Molecular_dynamics.md) runs. It *has*
to be supplied, otherwise VASP exits immediately after having started.
We recommend splitting long MD runs containing ab-initio calculations
into multiple calculations with NSW⪅20000. For
[ML_MODE](ML_MODE.md)=run larger values of NSW should be
possible, but consider setting
[ML_OUTBLOCK](ML_OUTBLOCK.md).

[IBRION](IBRION.md) != 0:

In all minimization algorithms (quasi-Newton, conjugate gradient, and
damped molecular dynamics) NSW defines the maximum number of ionic
steps.
