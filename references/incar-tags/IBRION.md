...
- no update
  - `IBRION`` = -1` (Avoid
    setting `IBRION`` = -1`
    with [`NSW`](NSW.md)` > 0` to prevent recomputing the same
...
- [Computing phonon modes](#computing-the-phonon-modes)
  ...
  - `IBRION`` = 6` finite
    differences with symmetry
  - `IBRION`` = 7`
    perturbation theory without symmetry (deprecated; use 8 instead)
  - `IBRION`` = 8`
...
- [Analyzing transition states](#analyzing-transition-states)
  ...
  - `IBRION`` = 44` [improved
    dimer method](https://vasp.at/wiki/index.php/Improved_dimer_method)
...
of the <a href="/wiki/Thermostats" class="mw-redirect"
title="Thermostats">thermostats</a>. The MD run performs
[NSW](NSW.md) timesteps of length [POTIM](POTIM.md).

Frequently, performing an
<a href="/wiki/Electronic_minimization" class="mw-redirect"
title="Electronic minimization">ab-initio calculations</a> in every step
of an MD simulation is too expensive so that [machine-learned force
...
## Structure optimization\[<a href="/wiki/index.php?title=IBRION&amp;veaction=edit&amp;section=2"
class="mw-editsection-visualeditor"
title="Edit section: Structure optimization">edit</a> \| (./index.php.md)\]
...
- RMM-DIIS (`IBRION`` = 1`)
  reduces the forces by linear combination of previous positions. It is
  the method of choice for larger systems (\>20 degrees of freedom) that
  are reasonably close to the ground-state structure.
- Conjugate gradient
  (`IBRION`` = 2`) finds the
  optimal step size along a search direction. It is a robust default
  choice but may need more iterations than RMM-DIIS.
- Damped molecular dynamics
  (`IBRION`` = 3`) runs a MD
  simulation with decreasing velocity of the ions. Use this for large
  systems far away from the minimum to get to a better starting point
...
## Computing the phonon modes\[<a href="/wiki/index.php?title=IBRION&amp;veaction=edit&amp;section=3"
class="mw-editsection-visualeditor"
title="Edit section: Computing the phonon modes">edit</a> \| (./index.php.md)\]

The second-order derivatives of the total energy
$E$ with respect to ionic positions
$R_{\alpha i}$ of ion $\alpha$ in
the direction $i$, is
computed using a first-order derivative of the
<a href="/wiki/Forces" class="mw-redirect" title="Forces">forces</a>
$F_{\beta j}$. Then, the dynamical matrix
$D_{\alpha i \beta j}$ is constructed, diagonalized,
and the phonon modes and frequencies of the system are reported in the
[OUTCAR](../output-files/OUTCAR.md) file and
...
(<a href="/wiki/Linear_response" class="mw-redirect"
title="Linear response">linear response theory</a>) or
...
## Analyzing transition states\[<a href="/wiki/index.php?title=IBRION&amp;veaction=edit&amp;section=4"
class="mw-editsection-visualeditor"
title="Edit section: Analyzing transition states">edit</a> \| (./index.php.md)\]

To study the kinetics of chemical reactions, one may want to construct
<a href="/wiki/Transition_states" class="mw-redirect"
title="Transition states">transition states</a> or follow the reaction
path. For the analysis of transition states the following methods are
available:

- Setting `IBRION`` = 40`, you
  can start from a transition state and monitor the energy along an
  intrinsic-reaction coordinate (IRC). The
  <a href="/wiki/IRC_calculations" class="mw-redirect"
  title="IRC calculations">IRC calculations</a> section describes this
  method.
- With the [improved dimer
  method](https://vasp.at/wiki/index.php/Improved_dimer_method)
  (`IBRION`` = 44`), you can
  search for a the transition state starting from an arbitrary structure
  in the investigated phase space.
- The [nudged elastic
...
## Interactively supplied positions and lattice vectors\[<a href="/wiki/index.php?title=IBRION&amp;veaction=edit&amp;section=5"
class="mw-editsection-visualeditor"
title="Edit section: Interactively supplied positions and lattice vectors">edit</a> \| (./index.php.md)\]
...
- With `IBRION`` = 11`, you
  can provide new structures via the standard input, cf. the
  [INTERACTIVE](INTERACTIVE.md) tag. For
...
## Related tags and articles\[<a href="/wiki/index.php?title=IBRION&amp;veaction=edit&amp;section=6"
class="mw-editsection-visualeditor"
...
title="Improved Dimer Method">Improved Dimer Method</a>, [Writing a
Python plugin](../tutorials/Plugins.md)
