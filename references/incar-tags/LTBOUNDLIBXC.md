...
Description: LTBOUNDLIBXC specifies whether or not the lower bound for
the Kohn-Sham kinetic-energy density $\tau_{\sigma}$ ($\tau_{\sigma}^{\textrm{W}}<\tau_{\sigma}$) is enforced
before $\tau_{\sigma}$ is used in a
[METAGGA](METAGGA.md) functional from Libxc.
...
$\tau_{\sigma}-\tau_{\sigma}^{\textrm{W}}$. If LTBOUNDLIBXC=.TRUE. in [INCAR](../input-files/INCAR.md), then
$\tau_{\sigma}=\max(\tau_{\sigma},\tau_{\sigma}^{\mathrm{W}})$ is applied before $\tau_{\sigma}$ is used in a meta-GGA functional from Libxc.

However, according to tests, for some of the most common meta-GGA
functionals like SCAN^([\[2\]](#cite_note-sun:prl:15-2)), a violation of
the lower bound is technically not a problem. Furthermore, it has been
observed that applying $\tau_{\sigma}=\max(\tau_{\sigma},\tau_{\sigma}^{\mathrm{W}})$ may possibly lead to very inaccurate forces and stress
tensor. Therefore, by default LTBOUNDLIBXC=.FALSE. and Libxc should be
compiled with the option `--disable-fhc` has explained
[here](../misc/Makefile.include.md).

Thus, the recommendation is to set LTBOUNDLIBXC=.TRUE. only in the case
convergence shows an erratic behavior. If this choice is made, then the
forces and stress tensor should be carefully monitored if a geometry
...
2.  [↑](#cite_ref-sun:prl:15_2-0) [J. Sun, A. Ruzsinszky, and J. P.
