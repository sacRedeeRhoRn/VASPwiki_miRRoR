...
NUCIND calculates the chemical shielding tensor $\sigma_{ij}(\mathbf{R})$ at positions $\textbf{R}$ off-nucleus, hence nucleus-independent chemical
shielding (NICS)
^([\[1\]](#cite_note-schleyer:1996-1)[\[2\]](#cite_note-chen:schleyer:2005-2)).
VASP calculates only from the plane wave grid, there is no one-center
augmentation.

When `NUCIND`` = True`, by default these are calculated on the fine FFT
grid [NGXF](NGXF.md) x [NGYF](NGYF.md) x
...
^([\[3\]](#cite_note-klod:kleinpeter:2001-3)[\[4\]](#cite_note-kleinpeter:klod:koch:2007-4)).
...
^([\[5\]](#cite_note-karadakov:horner:2013-5)):
...
shielding $\sigma_{iso}$ in the
selected plane. The plane is selected as a fraction `x` of the lattice
vector. Here, `x=0.5` corresponds to half of the primary lattice vector
$\mathbf{a}$. For the other lattice
vectors use `b=x` or `c=x`.
...
convention is followed ^([\[6\]](#cite_note-mason:ssn:1993-6))) by
inputting them as arguments into the functions, e.g.,
...
## References
1.  [↑](#cite_ref-schleyer:1996_1-0) [P. von Ragué Schleyer, C.
...
2.  [↑](#cite_ref-chen:schleyer:2005_2-0) [Z. Chen, C. S. Wannere, C.
...
3.  [↑](#cite_ref-klod:kleinpeter:2001_3-0) [S. Klod and E. Kleinpeter,
    *Ab initio calculation of the anisotropy effect of multiple bonds
    and the ring current effect of arenes—application in conformational
    and configurational analysis*, J. Chem. Soc., Perkin Trans. **2**,
    1893-1898 (2001).](https://doi.org/10.1039/B009809O)
4.  [↑](#cite_ref-kleinpeter:klod:koch:2007_4-0) [E. Kleinpeter, S.
    Klod, and A. Koch, *Visualization of through space NMR shieldings of
    aromatic and anti-aromatic molecules and a simple means to compare
    and estimate aromaticity*, J. Mol. Struct. THEOCHEM **811**, 45-60
    (2007).](https://doi.org/10.1016/j.theochem.2007.02.049)
5.  [↑](#cite_ref-karadakov:horner:2013_5-0) [P. Karadakov and K.
    Horner, *Magnetic Shielding in and around Benzene and
    Cyclobutadiene: A Source of Information about Aromaticity,
    Antiaromaticity, and Chemical Bonding*, J. Phys. Chem. A, **117**,
    518-523 (2013).](https://doi.org/10.1021/jp311536c)
6.  [↑](#cite_ref-mason:ssn:1993_6-0) [J. Mason, *Conventions for the
