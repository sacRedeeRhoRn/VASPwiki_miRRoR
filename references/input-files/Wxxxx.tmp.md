...
$W$ needed for BSE calculations and are
therefore fairly small compared to the
...
files, and then, if these are missing, the Wxxxx.tmp files. For small
isotropic (jellium-like) bulk systems, results with the Wxxxx.tmp might
be similar to the results obtained using the
