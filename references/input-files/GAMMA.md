...
# GAMMA
The GAMMA file is an input file used when
...
before calculating a new charge density. The file's format is structured
as follows:
...
occupations (KS occupations + GAMMA file) is as follows: The additional
occupation changes from the GAMMA can be a off-diagonal matrix
$\Delta N_{n n'}({\bf k})$, new natural
orbitals are found by a transformation matrix $V$. This transformation is found by diagonalizing the total
correlated density matrix:

$f'_{\nu {\bf k}} \delta_{\nu \nu'} = \sum_{n
n'} V_{\nu n} \left\[ f_{n {\bf k}} \delta_{n n'} + \Delta N_{n n'}
\right\] V^\*_{n' \nu'},$

$|\Psi'_{\nu {\bf k}}\rangle = \sum_n V_{\nu n}
|\Psi_{n {\bf k}}\rangle$

The new orbitals $|\Psi'_{\nu {\bf k}}\rangle$ together with $f'_{\nu {\bf k}}$ are then used to calculate the new charge density. For more
information see Ref.^([\[1\]](#cite_note-schueler:jpcm:30-1)) Eq.
(30)-(32).

The TRIQS software package^([\[2\]](#cite_note-parcollet:cpc:196-2))
...
VASP^([\[5\]](#cite_note-triqsdfttoolstutorial:web-5)[\[6\]](#cite_note-soliddmfttutorial:web-6)).
...
5.  [↑](#cite_ref-triqsdfttoolstutorial:web_5-0)
    [triqs.github.io/dft_tools/latest/tutorials.html#vasp-interface-examples
    (2024).](https://triqs.github.io/dft_tools/latest/tutorials.html#vasp-interface-examples)
6.  [↑](#cite_ref-soliddmfttutorial:web_6-0)
    [triqs.github.io/solid_dmft/tutorials/PrNiO3_csc_vasp_plo_cthyb/tutorial
    (2024).](https://triqs.github.io/solid_dmft/tutorials/PrNiO3_csc_vasp_plo_cthyb/tutorial.html)
