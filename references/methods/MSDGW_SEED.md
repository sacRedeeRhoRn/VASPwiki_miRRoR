...
MSDGW_SEED changes the random phases of the stochastic orbitals
determined by the compression algorithm.
[MSDGW_NXI](MSDGW_NXI.md) stochastic orbitals
$\lbrace |\xi_1\rangle,
\cdots,|\xi_{N_\xi}\rangle \rbrace$ are determined from the
original orbitals $\lbrace|\phi_1\rangle,\cdots,|\phi_N\rangle\rbrace$ by
drawing $N_\xi \times N$ pseudo numbers
$0\le \alpha_{i n}< 1$ and forming the
linear combinations $|\xi_i \rangle =
\sum_{n=1}^{N} e^{i 2\pi \alpha_{in}}|\phi_n\rangle$.
